...
class BaseModelSqlv2 {
  ...
  public async list(
    ...
    logger.log("start getAliasColObjMap")
    ...
    logger.log("start fixups")
    var columns = await this.model.getColumns()
    for (const col of columns) {
      if (col.uidt == UITypes.Links || col.uidt == UITypes.Rollup) {
        const columnOptions = await col.getColOptions()
        if (columnOptions.type === RelationTypes.MANY_TO_MANY) {
          ...
          for (const dataIndex in data) {
            ...
            toBeExpanded.push({ "dataIndex": dataIndex, "rowId": row['ncRecordId'] })
          }

          if (toBeExpanded.length == 0) {
            logger.log("Skipping list", col.title)
            continue
          }

          var mmListed = await this.multipleMmList({
            colId: col.id,
            parentIds: toBeExpanded.map(item => item["rowId"]),
          });

          for (const index in toBeExpanded) {
            var dataIndex = toBeExpanded[index]["dataIndex"]
            var rowId = toBeExpanded[index]["rowId"]
            var nested = []
            var listed = mmListed[index]
            for (const item of listed) {
              nested.push(item[parentTable.displayValue.title])
            }
            data[dataIndex][col.title] = nested
          }
        }
      }
    }
    ...
    setTimeout(function () {
      logger.log('Manual gc before', process.memoryUsage());
      global.gc();
      logger.log('Manual gc after', process.memoryUsage());
    }, 2000);
    return data?.map((d) => {
      d.__proto__ = proto;
      return d;
          ...
          if (this.isPg) {
            if (column.dt === 'bytea') {
              res[sanitize(column.id || column.column_name)] =
                this.dbDriver.raw(
                  `encode(??.??, '${column.meta?.format === 'hex' ? 'hex' : 'escape'
                  }')`,
        ...
        cnt = +(
          await this.execAndParse(
            this.dbDriver(this.getTnPath(mmModel.table_name))
              .where(
                `${this.getTnPath(mmModel.table_name)}.${mmChildColumn.column_name
                }`,
                rowId,
              )
  ...
  public async afterBulkUpdate(
    ...
    await Audit.insert({
      ...
      description: DOMPurify.sanitize(
        `${noOfUpdatedRecords} ${noOfUpdatedRecords > 1 ? 'records have' : 'record has'
  ...
  public async afterBulkDelete(
    ...
    await Audit.insert({
      ...
      description: DOMPurify.sanitize(
        `${noOfDeletedRecords} ${noOfDeletedRecords > 1 ? 'records have' : 'record has'
  ...
  public async afterBulkInsert(data: any[], _trx: any, req): Promise<void> {
    ...
    await Audit.insert({
      ...
      description: DOMPurify.sanitize(
        `${data.length} ${data.length > 1 ? 'records have' : 'record has'
  ...
  public async execAndParse(
    ...
    console.trace()
    logger.log("Start getColumns" + query)

    if (!this.model?.columns) {
      await this.model.getColumns();
    }
    logger.log("Start convertAttachmentType")
    ...
    logger.log("start convertDateFormat")
    ...
    logger.log("start convertUserFormat")
    ...
    logger.log("start substituteColumnIdsWithColumnTitles")

    // Start substituteColumnIdsWithColumnTitles
  ...
  public async convertAttachmentType(
    ...
    if (data) {
      ...
      logger.log("convertAttachmentType concat")

      const columns = this.model?.columns.concat(dependencyColumns ?? []);
      logger.log("start convertAttachmentType:columns")
      for (const col of columns) {
        if (col.uidt === UITypes.Lookup) {
      ...
      logger.log("start _convertAttachmentType")
...
export function _wherePk(primaryKeys: Column[], id: unknown | unknown[]) {
  ...
  for (let i = 0; i < primaryKeys.length; ++i) {
    if (primaryKeys[i].dt === 'bytea') {
      ...
      return (qb) => {
        qb.whereRaw(
          `?? = decode(?, '${primaryKeys[i].meta?.format === 'hex' ? 'hex' : 'escape'
          }')`,
