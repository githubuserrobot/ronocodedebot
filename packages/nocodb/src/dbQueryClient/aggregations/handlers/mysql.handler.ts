...
/** MySQL / MariaDB aggregation SQL — `CASE WHEN` based (no `FILTER`). */
export class MysqlAggregationHandler extends GenericAggregationHandler {
  ...
  protected numerical(ctx: AggregationSqlContext): Knex.Raw | undefined {
    const { knex, column, aggregation, column_query, subAggFrom, subAggCol } =
      ctx;
    const condnValue = ctx.condnValue;
