...
import { type AttachmentResType, UITypes } from 'nocodb-sdk';
...
export class CacheWarmingJob {
  ...
  async job(job: Job) {
    ...
    const fields = model.columns.map((c) => c.title).join(',');
    ...
    let data = await this.datasService.getDataList(context, {
      model,
